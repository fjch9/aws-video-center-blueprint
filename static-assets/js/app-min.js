...
jQuery(this).toggleClass("search-active");jQuery("#search-bar").slideToggle()});jQuery(document).on("click",".grid-system > a",function(event){event.preventDefault();jQuery('#'+jQuery(this).parent().data('id')).find('div.item').removeClass(jQuery(this).parent().find('a.current').data('class')).addClass(jQuery(this).data('class'))
jQuery('#'+jQuery(this).parent().data('id')).find('div.item [data-mh]').matchHeight();jQuery(this).parent().children('a').removeClass("current");jQuery(this).addClass("current");formatVideoSections()});var backtotop='#back-to-top';if(jQuery(backtotop).length){var scrollTrigger=100,backToTop=function(){var scrollTop=jQuery(window).scrollTop();if(scrollTop>scrollTrigger){jQuery(backtotop).addClass('show')}else{jQuery(backtotop).removeClass('show')}};backToTop();jQuery(window).on('scroll',function(){backToTop()});jQuery('#back-to-top').on('click',function(e){e.preventDefault();jQuery('html,body').animate({scrollTop:0},700)})}
...
var renderEventItem=function(parentId,video,videoBaseUrl){var url=videoBaseUrl+video.id;var className=jQuery(parentId).parent().parent().parent().find('.grid-system > .current').data('class')
return `  
...
var loadVideos=function(refreshUrl,retrieveLimit,parentId,videoBaseUrl){let url_video=refreshUrl+"?limit="+retrieveLimit;video_selector='#'+parentId+'-videos';jQuery.ajax({url:url_video,dataType:"json",success:function(data){if(data.length===0){jQuery('#'+parentId+'-section').addClass('hide')}else{jQuery('#'+parentId+'-section').removeClass('hide');jQuery.each(data,function(index,element){var domElement=document.getElementById("video-"+element.id);var liveText=jQuery('#video-'+element.id+' > div > div > .tag-live')
...
liveText.addClass('hide')}else{jQuery(video_selector).append(renderEventItem(parentId,element,videoBaseUrl))}
...
jQuery(document).ready(function(){setTimeout(function(){sidebarBackground=jQuery('.sidebarBg');sidebarBackground.css('width',sidebarBackground.css('width'))},250)})
