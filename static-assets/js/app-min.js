...
var loadVideos=function(refreshUrl,retrieveLimit,parentId,videoBaseUrl,itemGridClass){let url_video=refreshUrl+"?limit="+retrieveLimit;video_selector='#'+parentId+'-videos';jQuery.ajax({url:url_video,dataType:"json",success:function(data){if(data.length===0){jQuery('#'+parentId+'-section').addClass('hide')}else{jQuery('#'+parentId+'-section').removeClass('hide');jQuery.each(data,function(index,element){var domElement=document.getElementById("video-"+element.id);if(domElement){domElement.querySelector('.video-title').querySelector('a').innerHTML=element.title_s;domElement.querySelector('.post-summary').querySelector('p').innerHTML=element.summary_s;domElement.querySelector('.like-count').innerHTML=element.likeCount;domElement.querySelector('.dislike-count').innerHTML=element.dislikeCount;domElement.querySelector('.view-count').innerHTML=element.viewCount;if(element.type==='stream'){domElement.querySelector('.start-time').innerHTML=getDate(element.startDate_dt);domElement.querySelector('.end-time').innerHTML=getDate(element.endDate_dt)}
var liveText=jQuery('#video-'+element.id+' > div > div > .tag-live');if(element.liveNow){liveText.removeClass('hide')}else{liveText.addClass('hide')}}else{jQuery(video_selector).append(renderEventItem(element,videoBaseUrl,parentId,itemGridClass))}});dataIds=jQuery.map(data,function(e){return e.id});jQuery('div[data-video-id]').each(function(i,element){videoId=jQuery(this).data('video-id');if(dataIds.indexOf(videoId)<0){document.getElementById(element.id).remove()}})
